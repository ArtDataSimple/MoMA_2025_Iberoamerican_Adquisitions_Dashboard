...
def add_common_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    selected_country = st.sidebar.selectbox("País", country_options, index=0)
    ...
    text_query = st.sidebar.text_input("Buscar artista")

    filt = df.copy()

    if "País" in filt.columns and selected_country != "Todos":
        filt = filt[filt["País"] == selected_country]
...
def metric_row(df: pd.DataFrame):
    metrics = [
        ("Filas", len(df)),
        ("Artistas únicos", df["Artista"].nunique() if "Artista" in df.columns else 0),
        ("Países", df["País"].nunique() if "País" in df.columns else 0),
        ("Clasificaciones", df["Clasificación"].nunique() if "Clasificación" in df.columns else 0),
