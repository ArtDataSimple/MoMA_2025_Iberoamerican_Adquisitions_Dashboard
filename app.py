...
with st.expander("Diagnóstico de carga de datos"):
    st.write("Fuente cargada:", source_label)
    st.write("Directorio actual:", str(Path('.').resolve()))
    data_dir = Path("data")
    if data_dir.exists():
        st.write("Archivos en /data:", [p.name for p in sorted(data_dir.iterdir()) if p.is_file()])
    else:
        st.write("La carpeta /data no existe en este despliegue.")
    st.write("Archivos CSV/XLSX en raíz:", [p.name for p in sorted(Path('.').iterdir()) if p.is_file() and p.suffix.lower() in ['.csv', '.xlsx', '.xls']])
...
st.sidebar.markdown("---")
st.sidebar.info(
    "Sugerencia: guarda tu archivo como `data/3_MoMA_2025_Acquisitions_Clean.csv` dentro del repositorio para que la app lo cargue automáticamente en Streamlit Cloud."
)
