...
def load_dataframe(uploaded_file=None) -> pd.DataFrame:
    if uploaded_file is not None:
        ...
        found = next((p for p in DEFAULT_DATA_FILES if p.exists()), None)
        if found is None:
            raise FileNotFoundError(
                "No se encontró un archivo de datos por defecto. "
                "Sube un CSV/XLSX o añade `MoMA_final_transformed.xlsx` dentro de la carpeta `data/`."
            )
...
def plot_top_artists(df: pd.DataFrame):
    ...
    st.plotly_chart(fig, width="stretch", key="plot_top_artists")


def plot_unique_artists_by_country(df: pd.DataFrame):
    ...
    fig.update_traces(textposition="outside")
    st.plotly_chart(fig, width="stretch", key="plot_unique_artists_by_country")


def plot_living_vs_deceased(df: pd.DataFrame):
    ...
    fig.update_traces(textposition="outside")
    st.plotly_chart(fig, width="stretch", key="plot_living_vs_deceased")


def plot_hist_birth(df: pd.DataFrame):
    fig = px.histogram(
    ...
    st.plotly_chart(fig, width="stretch", key="plot_hist_birth")


def plot_hist_creation(df: pd.DataFrame):
    fig = px.histogram(
    ...
    st.plotly_chart(fig, width="stretch", key="plot_hist_creation")


def plot_forma_ingreso(df: pd.DataFrame):
    ...
    fig.update_traces(textposition="outside")
    st.plotly_chart(fig, width="stretch", key="plot_forma_ingreso")


def plot_named_donors(df: pd.DataFrame):
    ...
    fig.update_traces(textposition="outside")
    st.plotly_chart(fig, width="stretch", key="plot_named_donors")


def plot_classification(df: pd.DataFrame):
    ...
    fig.update_traces(textposition="outside")
    st.plotly_chart(fig, width="stretch", key="plot_classification")


def plot_department(df: pd.DataFrame):
    ...
    fig.update_traces(textposition="outside")
    st.plotly_chart(fig, width="stretch", key="plot_department")


def plot_cumulative_acq(df: pd.DataFrame):
    ...
    st.plotly_chart(fig, width="stretch", key="plot_cumulative_acq")


def plot_country_classification_heatmap(df: pd.DataFrame):
    ...
    st.plotly_chart(fig, width="stretch", key="plot_country_classification_heatmap")


def plot_box_creation_by_classification(df: pd.DataFrame):
    ...
    st.plotly_chart(fig, width="stretch", key="plot_box_creation_by_classification")


def plot_acq_year(df: pd.DataFrame):
    ...
    fig.update_traces(textposition="outside")
    st.plotly_chart(fig, width="stretch", key="plot_acq_year")
...
with tabs[0]:
    ...
    with c1:
        if {"País", "Clasificación"}.issubset(filtered.columns):
            plot_country_classification_heatmap(filtered)
    with c2:
        if "Forma Ingreso" in filtered.columns:
            plot_forma_ingreso(filtered)

    c3, c4 = st.columns(2)
    with c3:
        if "Artista" in filtered.columns:
            plot_top_artists(filtered)
    with c4:
        if "Fecha Adquisición" in filtered.columns:
            plot_cumulative_acq(filtered)
...
with tabs[1]:
    ...
    with c1:
        plot_top_artists(filtered)
        plot_living_vs_deceased(filtered)
        plot_hist_birth(filtered)
        plot_classification(filtered)
        plot_country_classification_heatmap(filtered)
        plot_box_creation_by_classification(filtered)
    with c2:
        plot_unique_artists_by_country(filtered)
        plot_hist_creation(filtered)
        plot_forma_ingreso(filtered)
        plot_named_donors(filtered)
        plot_department(filtered)
        plot_acq_year(filtered)
...
st.sidebar.markdown("---")
st.sidebar.info(
    "Sugerencia: para desplegar esta app, sube `app.py`, `requirements.txt` y tu archivo de datos a GitHub, y luego despliega en Streamlit Community Cloud."
)
